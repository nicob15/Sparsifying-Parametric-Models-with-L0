"Code adapted from pysindy https://github.com/dynamicslab/pysindy"

import numpy as np
from sklearn.utils.validation import check_is_fitted

from utils.axes import AxesArray
from utils.axes import comprehend_axes
...
class FourierLibrary():
    ...
    def get_feature_names(self, input_features=None):
        ...
        if self.interaction_terms:
            k = 0
            for i in range(self.n_output_features_ - self.n_interaction_terms):
                for j in range(self.n_output_features_ - self.n_interaction_terms - i - 1):
                    feat_name = str(feature_names[i] + " " + feature_names[j+1])
        ...
        if self.interaction_terms:
            if self.include_sin and self.include_cos:
                ...
                for i in range(self.n_output_features_):
                    n_interaction_terms += i
                self.n_interaction_terms = n_interaction_terms
                self.n_output_features_ = self.n_output_features_ + n_interaction_terms
            else:
    ...
    def transform(self, x_full):
        ...
        for x in x_full:
            ...
            if self.interaction_terms:
                ...
                for i in range(idx):
                    for j in range(self.n_output_features_ - self.n_interaction_terms - i - 1):
                        xp[..., idx + k] = xp[..., i] * xp[..., j + 1]
                        #print("xp[{}] = {}".format(idx+k, xp[..., idx + k]))
                        k += 1
...
if __name__ == "__main__":
    ...
    lib = FourierLibrary(n_frequencies=1, include_sin=True, include_cos=True, interaction_terms=True)
