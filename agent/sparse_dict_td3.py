...
from agent.feature_libraries.fourier_features import FourierLibrary
from agent.feature_libraries.polynomial_features import PolynomialLibrary
from agent.feature_libraries.generalized_features import GeneralizedLibrary
...
class Actor(nn.Module):
    def __init__(self, features_dim, max_action, lib, scale=1.0, action_dim=1, droprate=0.5, device='cuda'):
        ...
        torch.nn.init.normal_(self.actor_coef.weight, mean=0, std=0.01)
...
class TD3(object):
    def __init__(self, state_dim, action_dim, max_action, degree_pi=2, feature_scale=1.0, h_dim=256, tau=0.005,
                 reg_coeff=0.001, droprate=0.5, policy_type='polynomial', frequency=1.0, device='cuda'):

        if policy_type == 'poly_policy':
        ...
        if policy_type == "polyfourier_policy":
            ...
            x = np.ones((1, state_dim - 2))
            self.lib.fit(x)
            xpf = self.lib.transform(x)
            coef_dim = xpf.shape[1]
            print("polynomial + fourier policy with frequency ", frequency)
            print("with {} coefficients".format(coef_dim))
            print(self.lib.get_feature_names())
    ...
    def print_equations(self, m, ep):
        ...
        with open(os.path.join('figures/policy_m=' + str(m) + ".txt"), 'a') as file:
            file.write("\n")
            file.write("\n")
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, policy_noise=0.2, noise_clip=0.3,
        ...
        for it in range(iterations):
            ...
            if it % policy_freq == 0:
                ...
                # print("regularization loss", l0_norm_actor)
