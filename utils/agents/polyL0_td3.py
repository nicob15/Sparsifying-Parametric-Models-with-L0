...
class Actor(nn.Module):
    def __init__(self, coef_dim, max_action, poly, scale=1.0, action_dim=1, droprate=0.5, device='cuda'):
        super(Actor, self).__init__()

        self.actor_coef_1 = L0Dense(in_features=coef_dim, out_features=action_dim, bias=False, local_rep=False,
...
class TD3(object):
    def __init__(self, state_dim, action_dim, max_action, degree_pi=2, feature_scale=1.0, h_dim=256, tau=0.005,
                 reg_coeff=0.001, droprate=0.5, device='cuda'):
        ...
        self.actor = Actor(coef_dim, max_action, droprate=droprate, poly=self.poly_pi, scale=feature_scale,
                           action_dim=action_dim, device=device).to(device)
        self.actor_target = Actor(coef_dim, max_action, droprate=droprate, poly=self.poly_pi, scale=feature_scale,
                                  action_dim=action_dim, device=device).to(device)
    ...
    def print_equations(self, ep):
        ...
        w1 = w1[idx1, :]
        mask1 = mask1[:, idx1]
        coef1 = self.actor.poly.get_feature_names_out()
        ...
        with open(os.path.join('figures/policy.txt'), 'a') as file:
            file.write("\n")
            file.write("\n")
