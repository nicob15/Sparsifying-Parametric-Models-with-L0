...
class CodesignExperienceReplayBuffer(object):
    def __init__(self, state_dim, action_dim, design_dim, max_size=int(1e6), device='cuda'):
        self.max_size = max_size
        self.ptr = 0
        self.size = 0
        ...
        self.reward = np.zeros((max_size, 1))
        self.not_done = np.zeros((max_size, 1))
        self.design = torch.zeros((max_size, design_dim))

        self.device = device

    def add(self, state, action, next_state, reward, done, design):
        self.state[self.ptr] = state
        self.action[self.ptr] = action
        self.next_state[self.ptr] = next_state
        self.reward[self.ptr] = reward
        self.not_done[self.ptr] = done
        self.design[self.ptr] = design

        self.ptr = (self.ptr + 1) % self.max_size
        self.size = min(self.size + 1, self.max_size)

    # def normalize_rewards(self):
    #     r_max = self.reward.max()
    #     r_min = self.reward.min()
    ...
    def sample(self, batch_size):
        ...
        return (
            ...
            torch.FloatTensor(self.reward[ind]).to(self.device),
            torch.FloatTensor(self.not_done[ind]).to(self.device),
            self.design[ind].to(self.device),
        )
